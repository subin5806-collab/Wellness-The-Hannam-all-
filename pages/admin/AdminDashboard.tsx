...
export const AdminDashboard: React.FC = () => {
  ...
  if (!stats) return (
    <div className="min-h-screen flex items-center justify-center bg-[#FDFDFD]">
      <div className="font-serif text-gray-300 tracking-[0.3em] uppercase animate-pulse text-xs">Intelligence Loading...</div>
    </div>
  );

  const statItems = [
    { label: 'Reservations', value: stats.todayReservations, unit: 'SES', icon: Calendar, path: '/admin/reservations' },
    { label: 'Pending', value: stats.pendingSignatures, unit: 'WAI', icon: Clock, path: '/admin/members' },
    { label: 'Inquiries', value: stats.unprocessedInquiries, unit: 'NEW', icon: AlertCircle, color: stats.unprocessedInquiries > 0 ? 'text-red-500' : 'text-gray-900', path: '/admin/inquiries' },
  ...
  return (
    <div className="p-8 bg-[#FDFDFD] min-h-screen font-sans animate-fade-in">
      <div className="flex justify-between items-end mb-8 max-w-6xl mx-auto border-b border-gray-50 pb-6">
        <div>
          <h1 className="text-xl font-serif font-bold text-[#1A1A1A] uppercase tracking-wider">Dashboard</h1>
          <p className="text-[9px] font-black text-[#C9B08F] uppercase tracking-[0.4em] mt-1">Hannam Intelligence System</p>
        </div>
        <p className="text-[12px] num-clean text-gray-400">2025. 12. 18</p>
      </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          {statItems.map((item, i) => (
            <div 
              ...
              className="card-minimal p-4 flex flex-col justify-between h-[110px] cursor-pointer group"
            >
        ...
        <div className="grid grid-cols-12 gap-6">
          <div className="col-span-8 card-minimal p-6">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-[11px] font-black text-[#1A1A1A] border-l-2 border-[#C9B08F] pl-3 uppercase tracking-widest">Today's Schedule</h3>
            ...
            <div className="space-y-1">
               {reservations.length > 0 ? reservations.slice(0, 5).map(res => (
                 <div 
                   ...
                   className="flex items-center gap-4 p-3 rounded-lg hover:bg-[#FBFBFB] transition-all cursor-pointer group border border-transparent hover:border-gray-50"
                 >
          ...
          <div className="col-span-4 space-y-5">
             <div className="card-minimal p-5">
                <h3 className="text-[10px] font-black text-[#1A1A1A] uppercase tracking-widest mb-5">Low Balance</h3>
             ...
             <div className="card-minimal p-5">
                <h3 className="text-[10px] font-black text-[#1A1A1A] uppercase tracking-widest mb-5">Recent Inquiries</h3>
                <div className="space-y-3">
